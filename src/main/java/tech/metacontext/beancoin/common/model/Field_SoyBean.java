...
public class Field_SoyBean extends Field<Crop_SoyBean> {
   ...
   public Crop_SoyBean produce(Farmer farmer) {
      ...
      System.out.println("Total Farming Fee = " + totalFarmingFee);
      double totalFarmingFeeInBeancoin = BeanCoin.cashToBeanCoin(totalFarmingFee);
      System.out.println("Total Farming Fee in Beancoin = " + totalFarmingFeeInBeancoin);
      double s = farmer.getBeanCoin().spend(totalFarmingFeeInBeancoin);
      if (s < 0) {
         totalFarmingFeeInBeancoin += s;
